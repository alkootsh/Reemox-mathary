...
import { getCustomerStatement, getUserPreferences } from '../lib/firestoreService';
...
export default function CustomerStatementModal({ customer, onClose }: CustomerStatementModalProps) {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'credit_sale' | 'cash_sale' | 'payment' | 'return'>('all');
  const [expandedInvoiceId, setExpandedInvoiceId] = useState<string | null>(null);

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [ledger, setLedger] = useState<any[]>([]);
  const [customerDetails, setCustomerDetails] = useState<any>(customer);
  ...
  useEffect(() => {
    async function loadPrefs() {
      try {
        const prefs = await getUserPreferences(userEmail, 'customer_statement');
        if (prefs && prefs.visible && prefs.order) {
          setVisibleKeys(prefs.visible);
          setOrderedKeys(prefs.order);
        }
      } catch (err) {
        console.warn("Failed to load customer statement preferences", err);
      }
    }
    loadPrefs();
  }, [userEmail]);
  ...
  useEffect(() => {
    async function fetchStatement() {
      try {
        setLoading(true);
        setError(null);
        const res = await getCustomerStatement(customer.id, companyId);
        if (res && res.success) {
  ...
  const filteredLedger = useMemo(() => {
    return ledger.filter(entry => {
      ...
      // Type filter
      if (typeFilter === 'credit_sale') {
        if (entry.type !== 'SALE' || entry.paymentMethod !== 'credit') return false;
      } else if (typeFilter === 'cash_sale') {
        if (entry.type !== 'SALE' || entry.paymentMethod === 'credit') return false;
  ...
  const totalInvoiced = useMemo(() => {
    // Total Debits from sales or direct debit transactions
    return filteredLedger.reduce((sum, entry) => sum + (entry.debit || 0), 0);
  }, [filteredLedger]);

  const totalPaid = useMemo(() => {
    // Total Credits from payments, returns or immediate sales payments
    return filteredLedger.reduce((sum, entry) => sum + (entry.credit || 0), 0);
  }, [filteredLedger]);

  const isFiltered = Boolean(fromDate || toDate || invoiceSearch || itemSearch || typeFilter !== 'all');
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-black text-text-main flex flex-wrap items-center gap-2">
                ...
                <span className="text-gold break-all">{customerDetails.name}</span>
              </h2>
              <p className="text-xs text-text-dim">عرض الفواتير المسجلة، الأصناف المباعة، وسجل الديون والمسحوبات والمدفوعات</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="cash_sale">مبيعات نقدية (كاش)</option>
                  <option value="credit_sale">مبيعات آجلة (على الحساب)</option>
                  <option value="payment">سداد ومدفوعات</option>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6 text-text-main printable-statement font-sans flex-1">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4">
              ...
              <p className="text-xs text-text-dim">جاري جلب وعرض كشف الحساب التفصيلي من قاعدة البيانات...</p>
            </div>
          ) : error ? (
            ...
            <>
              ...
              <div className="flex justify-between items-start border-b border-border pb-4">
                <div>
                  ...
                  <p className="text-xs text-text-dim mt-0.5">كشف حساب عميل مفصل وخلاصة الحركة المالية</p>
                ...
                <div className="text-left font-mono text-xs text-text-dim space-y-1">
                  ...
                  <p>العميل: <strong className="text-gold font-bold">{customerDetails.name}</strong></p>
                  {customerDetails.phone && <p>الهاتف: <strong className="text-text-main">{customerDetails.phone}</strong></p>}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                ...
                <div className="bg-card2 border border-border p-3 rounded-2xl">
                  <span className="text-[11px] text-text-dim block">إجمالي المسحوبات (مدين)</span>
                  <span className="text-sm font-black font-mono text-danger">{(totalInvoiced || 0).toLocaleString()} ج.م</span>
                </div>

                <div className="bg-card2 border border-border p-3 rounded-2xl">
                  <span className="text-[11px] text-text-dim block">إجمالي السداد (دائن)</span>
                  <span className="text-sm font-black font-mono text-success">{(totalPaid || 0).toLocaleString()} ج.m</span>
                </div>
              ...
              <div className="space-y-3">
                <h3 className="font-bold text-sm text-text-main flex items-center justify-between">
                  ...
                  <span className="text-xs text-text-dim font-mono font-normal">عرض تسلسلي زمني متكامل</span>
                </h3>

                {filteredLedger.length === 0 ? (
                  <div className="text-center py-8 bg-card2/50 rounded-2xl border border-border text-text-dim text-xs">
                    لا توجد فواتير أو عمليات مطابقة للبحث المحدد لهذا العميل
                  </div>
                ) : (
                                    ...
                                    case 'type':
                                      return (
                                        <td key={colKey} className="p-3 text-[11px] font-bold text-text-dim">
                                          {entry.type === 'SALE' ? 'فاتورة بيع' : 
                                           entry.type === 'PAYMENT' ? 'سند تحصيل' : 
                                           entry.type === 'RETURN' ? 'مرتجع' : 'حركة'}
                                        </td>
                                      );
                                    case 'notes':
                                      return (
                                        <td key={colKey} className="p-3">
                                          <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-text-dim">
                                              {entry.type === 'SALE' ? 'فاتورة بيع مبيعات' : 
                                               entry.type === 'PAYMENT' ? 'سند تحصيل نقدية' : 
                                               entry.type === 'RETURN' ? 'إذن مرتجع مبيعات' : 'حركة محاسبية'}
                                            </span>
                                          </div>
                                        </td>
                                      );
